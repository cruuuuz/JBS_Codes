...
class Production:
    ...
    @staticmethod
    def Beef_Production_Data(proxies: dict, year: int, api_key: str) -> pd.DataFrame:
        ...
        short_descs = [
            "BEEF, SLAUGHTER, COMMERCIAL - PRODUCTION, MEASURED IN LB",
            "BEEF, SLAUGHTER, COMMERCIAL, FI - PRODUCTION, MEASURED IN LB",
            "VEAL, SLAUGHTER, COMMERCIAL - PRODUCTION, MEASURED IN LB",
            "VEAL, SLAUGHTER, COMMERCIAL, FI - PRODUCTION, MEASURED IN LB"
        ]

        urls = [
            f'https://quickstats.nass.usda.gov/api/api_GET/?key={api_key}&source_desc=SURVEY&short_desc={quote(desc, safe=",")}&year__GE={year}'
        ...
        print(f"Tempo de requisição total do Beef Production: {time.time() - t0:.2f} s")
    ...
    @staticmethod
    def Pork_Production_Data(proxies: dict, year: int, api_key: str) -> pd.DataFrame:
        ...
        short_descs = [
            "PORK, SLAUGHTER, COMMERCIAL - PRODUCTION, MEASURED IN LB",
            "PORK, SLAUGHTER, COMMERCIAL, FI - PRODUCTION, MEASURED IN LB"
        ]
        ...
        with ThreadPoolExecutor(max_workers=8) as executor:
            fetched_data = list(executor.map(fetch_data, urls))
        print(f"Tempo de requisição total do Pork Production: {time.time() - t0:.2f} s")
