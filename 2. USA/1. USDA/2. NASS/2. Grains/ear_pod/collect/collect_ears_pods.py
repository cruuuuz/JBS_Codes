...
class EarPodCollector:
    @staticmethod
    def collect_ear_pod(commodity: str, proxies: dict, year: int, api_key: str) -> pandas.DataFrame:
        ...
        for name, desc in INDICATORS[commodity].items():
            #Pega os dados a nivel de estados
            url_state = (
            ...
            response = requests.get(url_state, proxies=proxies)
            ...
            for item in conteudo_json:
                ...
                df.insert(1, "WEEK_ENDING_DATE", None)
                dfs.append(df)
        ...
        if df_indicators:
            ...
            # df_indicators.columns = ['REPORT_DATE', 'WEEK_ENDING_DATE', 'REFERENCE_WEEK', 'YEAR', 'SECTOR', 'GROUP_DESCRIPTION', 'COMMODITY_DESCRIPTION', 'SHORT_DESCRIPTION', 'UNIT', 'STATE', 'STATE_NAME', 'VALUE']
