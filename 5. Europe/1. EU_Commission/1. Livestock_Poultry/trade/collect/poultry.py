...
class Poultry:
    ...
    def get_Prices(base_url: str, proxies: dict, date: str):
        ...
        df_geral = df_geral[['endDate', 'beginDate', 'weekNumber', 'memberStateCode', 'memberStateName', 'productName', 'priceType', 'price', 'unit']]
        df_geral.columns = ['WEEK_ENDING_DATE', 'WEEK_START_DATE', 'WEEK_NUMBER', 'COUNTRY_CODE', 'COUNTRY_NAME', 'PRODUCT_CUT', 'PRICE_TYPE', 'PRICE', 'UNIT']
        ...
        df_geral['PRODUCT_CUT'] = df_geral['PRODUCT_CUT'].str.upper()
        df_geral['PRICE_TYPE'] = df_geral['PRICE_TYPE'].str.upper()
        df_geral['UNIT'] = df_geral['UNIT'].str.upper()
        df_geral.insert(5, "PRODUCT", "POULTRY")
